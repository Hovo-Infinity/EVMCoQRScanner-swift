...
/**
 Class to store language tag associated with LanguadeData
 */
 ...
 */
@property(class,nonnull,readonly) TagInfo*  TAG_00_LANGUAGE_PREFERANCE NS_SWIFT_NAME(TAG_00_LANGUAGE_PREFERANCE);//("00", "^[a-zA-Z]{2}$", true),
/**
 * Alternate Merchant Name: Should be the “doing business as” name for the merchant in merchant's local language.
 */
@property(class,nonnull,readonly) TagInfo* TAG_01_ALTERNATE_MERCHANT_NAME NS_SWIFT_NAME(TAG_01_ALTERNATE_MERCHANT_NAME);//("01", "^.{1,25}$", true),
/**
 * Alternate Merchant City: Should be the city in which the merchant transacts in the merchant’s local language.
 */
@property(class,nonnull,readonly) TagInfo* TAG_02_ALTERNATE_MERCHANT_CITY NS_SWIFT_NAME(TAG_02_ALTERNATE_MERCHANT_CITY);//("02", "^.{1,15}$", false);
